...
def draw_bounding_box(cvImg, R, T, mesh_or_bbox, intrinsics, color):
    thickness = 2
...
def draw_pose_axis(cvImg, R, T, mesh_or_bbox, intrinsics):
    thickness = 2
    if isinstance(mesh_or_bbox, trimesh.Trimesh):
        bbox = mesh_or_bbox.bounding_box_oriented.vertices
    else:
...
def draw_pose_info_text(cvImg, R, T, position=0, color=(0,255,0)):
    ...
    #
    info = ("Euler angles: %.2f, %.2f, %.2f" % (ak, aj, ai))
    ...
    info = ("Translations: %.2f, %.2f, %.2f" % (T[0], T[1], T[2]))
...
def draw_pose_metric_text(cvImg, error_3d, error_2d, estecm_valid, diameter, color=(0,255,0)):
    ...
    cvImg = cv2.putText(cvImg, info, (xPos, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 10, cv2.LINE_AA) # black background
    cvImg = cv2.putText(cvImg, info, (xPos, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1, cv2.LINE_AA) 
    # 
    info = ("CSM: %d" % (estecm_valid))
    cvImg = cv2.putText(cvImg, info, (xPos, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 10, cv2.LINE_AA) # black background
    cvImg = cv2.putText(cvImg, info, (xPos, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1, cv2.LINE_AA) 
    return cvImg
...
def visualize_pred(meta_info, pred, sym_types, meshes, bboxes_3d, diameters):
    ...
    for score, cls_id, R, T in pred:
        ...
        err_3d, err_2d = compute_pose_diff(
            surfacePts[cls_id], K,
            gtR, gtT, R, T, isSym)
        estecm_valid = compute_pose_validation_vespa(gtR, gtT, R, T, diameters[cls_id])
        # print(err_3d)
        # print(err_2d)
        # print(estecm_valid)
        ...
        try:
            ...
            if text_drawing:
                ...
                cvImg = draw_pose_metric_text(cvImg, err_3d, err_2d, estecm_valid, diameters[cls_id], color=(0,255,0))
        except:
            pass
...
def compute_pose_validation_vespa(gtR, gtT, predR, predT, diameter):
    # 
    ai, aj, ak = transforms3d.euler.mat2euler(gtR, axes='szyx')
    bi, bj, bk = transforms3d.euler.mat2euler(predR, axes='szyx')
    #
    diff_angle_z = np.abs(ai - bi) * 180 / np.pi
    diff_angle_y = np.abs(aj - bj) * 180 / np.pi
    diff_angle_x = np.abs(ak - bk) * 180 / np.pi
    #
    if diff_angle_z > 180:
        diff_angle_z = 360 - diff_angle_z
    if diff_angle_y > 180:
        diff_angle_y = 360 - diff_angle_y
    if diff_angle_x > 180:
        diff_angle_x = 360 - diff_angle_x
    #
    diff_trans_ratio = (np.abs(gtT - predT) / diameter).reshape(-1).tolist()
    #
    if diff_angle_x > 20 or diff_angle_y > 20 or diff_angle_z > 20 or \
        diff_trans_ratio[0] > 0.075 or diff_trans_ratio[1] > 0.075 or diff_trans_ratio[2] > 0.15:
        return 0
    return 1
