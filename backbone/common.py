...
class CustomConv2d(nn.Conv2d):
    def __init__(
        ...
        u = x/sigma
        v = y/sigma
        gs = torch.exp(-(u*u+v*v)/2)
        self.register_buffer("kernel_smoother", gs/gs.sum()) # register after normalize

    def forward(self, input):
        oc, ic, kw, kh = self.weight.shape
        smoothed_weight = F.conv2d(self.weight.view(1,-1,kw,kh),
                self.kernel_smoother.repeat(oc*ic,1,1,1),
                padding=int(kw/2),
                groups=oc*ic).reshape(oc, ic, kw, kh)
        new_weight = self.weight - smoothed_weight
        return super(CustomConv2d, self)._conv_forward(input, new_weight, self.bias)
...
class PreConvBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_bn:
            ...
            #self.bn = FrozenBatchNorm2d(in_channels)
        if self.activate:
...
class IBN(nn.Module):
    ...
    def __init__(self,
        ...
        if self.inst_first:
            ...
            #self.batch_norm = FrozenBatchNorm2d(h2_channels)
        else:
            self.batch_norm = nn.BatchNorm2d(num_features=h1_channels)
            #self.batch_norm = FrozenBatchNorm2d(h1_channels)
            self.inst_norm = nn.InstanceNorm2d(
