    ...
    for clsid in range(classNum):
        ...
        for filename, item in predictions.items():
            ...
            if len(pred) > 0:
                ...
                #
                errors_vespa.append(compute_pose_validation_vespa(R1, T1, R2, T2, mesh_diameters[clsid]))
                # 
        ...
        estec_m = 0
        if np.array(errors_vespa).sum() > 0:
            estec_m = 100 * np.array(errors_vespa).sum() / len(errors_vespa)
        print("Accuracy in custom metric: %.2f (%d)" % (estec_m, len(errors_vespa)))
        #
...
def remap_predictions(internal_K, internal_width, internal_height, keypoints_3d, meta, preds):
    ...
    for idx in range(len(preds)):
        ...
        new_preds.append([score, clsid, newR, newT])
    # 
    return new_preds
...
if __name__ == "__main__":
    ...
    image_path = "/data/vespa_syn_0327/train/000000/rgb/000001.png"
